function [header,m,channels] = MEA_load_bin_20210304(binfile,plt, convertOption,inputFolder,outputFolder)
...
% cd 'C:\Users\Windows\Dropbox (Cambridge University)\NOG MEA Data\MEA Data Mecp2 Project Jan 2019-\RAW files'
cd(inputFolder)
%% initialize
...
uVf=findstr(m,'µV/AD'); %AD changed to what it says in original nictera script
...
%AD getting error message that matrix dimensions for multiplication dont match this could be why?
...
if plt == 1 %AD added '== 1' as it just said 'if plt'
    ch=82;
    figure
    t=1:size(m,2);
    t=t/fs;
    size(m)
    f=find(channels==ch)
    plot(t,m(f(1),:),'k') %plot 1st 100ms with: plot(t(1:length(t)/7200),m(f(1),1:length(m)/7200),'k')
end;

%% save
sprintf('Saving data ...')

%AD: change directory to desired output location
% cd 'C:\Users\Windows\Dropbox (Cambridge University)\NOG MEA Data\MEA Data Mecp2 Project Jan 2019-\MAT files';
cd(outputFolder)
...
%AD: change directory back to script location
