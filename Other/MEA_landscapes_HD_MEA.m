...
MEAks     = HDMEA1449ks;
MEAbinary = HD_MEA_binary_single;
% update distance and parameter space for cell n and 100 sims
...
% ks statistics
[ks ksi]  = (min(MEAks,[],3));
% squeeze when there are multiple cultures (dim. 1 is >1)
% ks        = squeeze(ks);
% ksi       = squeeze(ksi);
% use reshape when one culture
ks          = reshape( ks, size(ks, 1), size(ks,2),size(ks,4) );
ksi         = reshape( ksi, size(ksi, 1), size(ksi,2),size(ksi,4) );
...
% for single culture:
h = boxplot([e(:,rearrange);e(:,rearrange)],'plotstyle','compact','color',[0.3 0.3 0.3]); 
...
% single culture
networktable = table(modeltype,mean([e;e])',std([e;e])',mean([p1;p1])',std([p1;p1])',mean([p2;p2])',std([p2;p2])','VariableNames',...
    {'Rule','Mean E','Std E','Mean P1','Std P1','Mean P2','Std P2'});
disp(networktable);
