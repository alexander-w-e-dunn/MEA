...
%         try
            ...
            if strcmp(method,'cwt')
                fileName = strcat(files(file).name(1:end-4), '_cSpikes_L',num2str(L),'_RP',num2str(refPeriod_ms), '.mat');
                if ~exist(fileName)
                    [spikeMatrix,~,thresholds] = getSpikeMatrixAlex(data, 'cwt', multiplier, L,fs,refPeriod_ms); %AD added, need to adjust loss parameter
            ...
            elseif strcmp(method,'Manuel')
                fileName = strcat(files(file).name(1:end-4), '_mSpikes_',num2str(multiplier),'_RP',num2str(refPeriod_ms), '.mat');
                if ~exist(fileName)
                    [spikeMatrix,~,thresholds] = getSpikeMatrixAlex(data, 'Manuel', multiplier, L,fs,refPeriod_ms); %AD added, need to adjust loss parameter
                    spikeMatrix(:,find(channels == 15)) = zeros(size(spikeMatrix(:,find(channels == 15)))); %remove spikes from ref channel
                    mSpikes = sparse(spikeMatrix);
                    %toc
                    % save(fileName, 'mSpikes', 'tSpikes', 'pSpikes');
                    save(fileName, 'mSpikes','channels','thresholds','refPeriod_ms','method','fs','multiplier');
                else
                    disp('spikes already detected')
                end
                progressbar(file/length(files),[],[]);
            ...
            elseif strcmp(method,'abs')
                fileName = strcat(files(file).name(1:end-4), '_aSpikes_',num2str(multiplier),'_RP',num2str(refPeriod_ms), '.mat');
                if ~exist(fileName)
                    [spikeMatrix,~,thresholds] = getSpikeMatrixAlex(data, 'abs', multiplier, L,fs,refPeriod_ms);
                    spikeMatrix(:,find(channels == 15)) = zeros(size(spikeMatrix(:,find(channels == 15)))); %remove spikes from ref channel
                    aSpikes = sparse(spikeMatrix);
                    %toc
                    save(fileName, 'aSpikes','channels','thresholds','refPeriod_ms','method','fs','multiplier');
                else
                    disp('spikes already detected')
                end
