function [burstMatrix, burstTimes, burstChannels, ISInTh] = burstDetect(spikeMatrix, method, samplingRate, N, minChannel)
...
if strcmp(method, 'nno')
    ...
    min_nspikes = N; % minimum number of spikes to count as burst
...
if strcmp(method, 'surprise') 
    spikeTimes = findSpikeTimes(spikeMatrix, 'seconds', samplingRate);
...
%     samplingRate=sampling_fr
    ...
    ISI_N = ISInTh; % Added RT
    
    %[Burst SpikeBurstNumber] = BurstDetectISIn(Spike, N, ISInTh); % Error in BurstDecetISIn
    ...
    for bb = 1:length(Burst.S)
        ...
        burstCell{bb} = sparse(logical(spikeMatrix(T_start_frame:T_end_frame, :)));
    end 
