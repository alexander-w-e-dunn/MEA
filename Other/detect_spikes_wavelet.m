function TE = detect_spikes_wavelet(...
    Signal, SFr, Wid, Ns, option, L, wname, PltFlg, CmtFlg, refPeriod_ms)
...
TE = parse(Index,SFr,Wid,refPeriod_ms);
...
function fcn = parse(Index,SFr,Wid,refPeriod_ms);
...
% Refract = 1.0 * Wid(2);    %[ms] the refractory period -- can't resolve spikes 
Refract = refPeriod_ms;    %that are closer than Refract;
