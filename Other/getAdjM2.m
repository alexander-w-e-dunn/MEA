function adjM = getAdjM2(spikes,method,downSample,lag,fs)
...
% downsample
if downSample ~= 0
   lSpikes = length(spikes);
   spikes = downSampleSum(spikes, downSample);
   lSpikes_ds = length(spikes);
   spikes(spikes == 2) = 1;
end
if downSample == 0
    lSpikes = length(spikes);
...
%     fs = 25000;
    ...
    for channelPairNum = 1:size(combChannel, 1)
        ...
        N1v = sum(electrode_1); 
        N2v = sum(electrode_2); 
        spike_times_1 = find(electrode_1 >= 1) / fs_ds; 
        spike_times_2 = find(electrode_2 >= 1) / fs_ds;
