function [cb] = makeHeatMap_square(spikeMatrix, option,channels,sample_rate) 
    ...
    % makes electrodes with 0 spikes NaN
    spikeCount(find(spikeCount == 0)) = NaN; % log10(0) = -Inf so set to NaN if fire rate is 0
    
    if strcmp(option,'rate')
        spikeCount = spikeCount / (size(spikeMatrix, 1) / sample_rate);
    end 
    ...
    if strcmp(option,'logr')
        spikeCount = log10( spikeCount / (size(spikeMatrix,1) / sample_rate) );
    end 
    ...
    elseif strcmp(option, 'logr')
        %ylabel(cb, 'Log10 spike count')   
        ylabel(cb, 'Spike rate (Hz)')
        ylimit_cbar = log10(200); % 20 Hz limit
        ymin_cbar   = log10(0.01);
        caxis([ymin_cbar,ylimit_cbar]) %set colorbar axis limits; also adjusts colour
        cb.Ticks = [log10(0.01) log10(0.1) log10(0.5) log10(1) log10(2) log10(5) log10(10) log10(20) log10(50) log10(100) log10(200)];%(start,end,number of numbers)
