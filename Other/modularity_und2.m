function [Q]=modularity_und2(gamma, A)
% note in this function, Q is the inverse of modularity coefficient in
% modularity_und
...
%   Sep 2021: Edited for use with simulated annealing to find global maximum Q
persistent step
...
Q=~(s-s.').*B/m;
Q=sum(Q(:));                            % 1/modularity so can use simulated annealing to get global maximum Q by getting global min 1/Q
Q=1./Q;
...
step = step + 1;
% get mean size of modules
modIDs = unique(Ci); % module IDs
for mod = 1:length(modIDs)
    modSize(mod) = sum(Ci==modIDs(mod)); % number of nodes in module
end
% array contains, modularity coeff at each step; gamma; number of modules;
% mean module size (should be n nodes / n modules); community affiliation
array(step,1) = 1./Q;                     
array(step,2) = gamma;
array(step,3) = max(Ci);
array(step,4) = mean(modSize);