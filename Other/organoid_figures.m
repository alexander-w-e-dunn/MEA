...
scrptDir = 'C:\Users\alexd\OneDrive - University of Cambridge\Cam\PhD\Project\Data&CodeBackup\Scripts';
addpath(genpath(scrptDir));
%get data
    % lab PC
% cd 'D:\MECP2_2019_AD\Scripts_and_Output\S1.2.File_Conversion_Output'
    % hp laptop
cd 'C:\Users\alexd\OneDrive - University Of Cambridge\Cam\PhD\Project\Data&CodeBackup\Data'
...
hold on
notBoxPlot([dataD],4,'style','sdline')
notBoxPlot([dataC],1,'style','sdline')
notBoxPlot([dataB],2,'style','sdline')
notBoxPlot([dataA],3,'style','sdline')
...
xlim([0.5 4.5])
labs = ['C9-L1'; 'H-L2 '; 'H-L1 '; 'C9-L2'];
labs = labs([3,2,1,4],:)
xticklabels(labs)
set(gca,'XMinorTick','off','YMinorTick','on');

a.Children(5).FaceColor = 'white';
a.Children(5).EdgeColor = '#A2142F';
% a.Children(5).FaceAlpha = .5;
a.Children(3).Color = '#A2142F';
a.Children(3).MarkerFaceColor = 'white';
a.Children(2).Color = '#A2142F';
a.Children(4).Color = '#A2142F';

a.Children(6).Color = '#B7B7B7';
a.Children(7).MarkerEdgeColor = '#B7B7B7';
a.Children(7).MarkerFaceColor = 'white';
a.Children(8).Color = '#B7B7B7';
a.Children(9).FaceColor = '#D3D3D3';
a.Children(9).FaceAlpha = 1;
a.Children(9).EdgeColor = '#B7B7B7';

a.Children(10).Color = '#313131';
a.Children(11).Color = '#313131';
a.Children(12).Color = '#313131';
a.Children(13).FaceColor = '#A9A9A9';
a.Children(13).EdgeColor = '#313131';

a.Children(14).MarkerEdgeColor = '#FF837F';
a.Children(14).MarkerFaceColor = 'white';
a.Children(15).MarkerFaceColor = 'white';
a.Children(15).Color = '#FF837F';
a.Children(16).Color = '#FF837F';
a.Children(17).EdgeColor = '#FF837F';
a.Children(17).FaceColor = 'black';
a.Children(14).Color = '#FF837F';

% saveas(f,'C:\Users\alexd\OneDrive - University of Cambridge\Cam\PhD\Manuscripts\PhD_Thesis\Chapter_Organoid\Figures\plots\smallworld','svg');
% saveas(f,'C:\Users\alexd\OneDrive - University of Cambridge\Cam\PhD\Manuscripts\PhD_Thesis\Chapter_Organoid\Figures\plots\smallworld','fig');
values = {output.SW2};
idx = find(~cellfun(@isempty,values));
values = cell2mat(values(idx));
groups1 = {output.grp};
groups1 = groups1(idx);

% Convert string groups to numeric values
numeric_groups = grp2idx(groups1)';

% take only group 1:4
idx = find(numeric_groups<5);
values = values(idx);
groups = groups(idx);
numeric_groups = numeric_groups(idx);

% Calculate the mean for each group
mean_values = accumarray(numeric_groups', values', [], @mean);

% Calculate the standard error for each group
group_counts = histcounts(numeric_groups, 1:max(numeric_groups)+1);
std_values = accumarray(numeric_groups', values', [], @std);
stderr_values = std_values ./ sqrt(group_counts)';

% Perform a one-way ANOVA
[p_value, table, stats] = anova1(values, numeric_groups, 'off');

% Print the mean and standard error for each group
for i = 1:length(mean_values)
    fprintf('Group %d: Mean = %.2f, Standard Error = %.2f\n', i, mean_values(i), stderr_values(i));
end
...
for i = 1:length(ind)
    dataE(i) = output(ind(i)).RC2;
end

clear ind; ind = find(groups =='D');
for i = 1:length(ind)
    dataD(i) = output(ind(i)).RC2;
end

clear ind; ind = find(groups =='C');
for i = 1:length(ind)
    dataC(i) = output(ind(i)).RC2;
end

clear ind; ind = find(groups =='B');
for i = 1:length(ind)
    dataB(i) = output(ind(i)).RC2;
end

clear ind; ind = find(groups =='A');
for i = 1:length(ind)
    dataA(i) = output(ind(i)).RC2;
end
...
xticks(1:5)
xticklabels({'CS30','EpiC','WTsli42','D','CS30 older'})
ylabel('Normalised rich club coef.')
...
a.Children(5-1).FaceColor = 'white';
a.Children(5-1).EdgeColor = '#A2142F';
% a.Children(5).FaceAlpha = .5;
a.Children(3-1).Color = '#A2142F';
a.Children(3-1).MarkerFaceColor = 'white';
a.Children(2-1).Color = '#A2142F';
a.Children(4-1).Color = '#A2142F';

a.Children(6-1).Color = '#B7B7B7';
a.Children(7-1).MarkerEdgeColor = '#B7B7B7';
a.Children(7-1).MarkerFaceColor = 'white';
a.Children(8-1).Color = '#B7B7B7';
a.Children(9-1).FaceColor = '#D3D3D3';
a.Children(9-1).FaceAlpha = 1;
a.Children(9-1).EdgeColor = '#B7B7B7';

a.Children(10-1).Color = '#313131';
a.Children(11-1).Color = '#313131';
a.Children(12-1).Color = '#313131';
a.Children(13-1).FaceColor = '#A9A9A9';
a.Children(13-1).EdgeColor = '#313131';

a.Children(14-1).MarkerEdgeColor = '#FF837F';
a.Children(14-1).MarkerFaceColor = 'white';
a.Children(15-1).MarkerFaceColor = 'white';
a.Children(15-1).Color = '#FF837F';
a.Children(16-1).Color = '#FF837F';
a.Children(17-1).EdgeColor = '#FF837F';
a.Children(17-1).FaceColor = 'black';
a.Children(14-1).Color = '#FF837F';

f=gcf;
f.Position=[1591         623         329         373];
% 
% saveas(f,'C:\Users\alexd\OneDrive - University of Cambridge\Cam\PhD\Manuscripts\PhD_Thesis\Chapter_Organoid\Figures\plots\rc','svg');
% saveas(f,'C:\Users\alexd\OneDrive - University of Cambridge\Cam\PhD\Manuscripts\PhD_Thesis\Chapter_Organoid\Figures\plots\rc','fig');
clear idx
values = {output.RC2};
idx = find(~cellfun(@isempty,values));
values = cell2mat(values(idx));
...
%% RE ORDERED firing rate * SD * cell lines as separate lines RE ORDERED
clear all; close all
...
A = [scores1{3,1}; scores2{3,1}; scores3{3,1}];
B = [scores1{2,1}; scores2{2,1}; scores3{2,1}];
C = [scores1{1,1}; scores2{1,1}; scores3{1,1}];
D = [scores1{4,1}; scores2{4,1}; scores3{4,1}];
E = [scores1{5,1}; scores2{5,1}; scores3{5,1}];

f1 = figure;
colours = {[0.6350 0.0780 0.1840];[0.7 0.7 0.7];[0.2 0.2 0.2];	[1 0.5 0.5]};
colours = colours([3 2 1 4]);
hold on
for i  = 1:size(A,1)
    scatter((i*ones(1,length(A)))+0.15,A(i,:),50,colours{1,1},'o','jitter', 'off', 'jitterAmount', 0.3)
end
...
labs = ['C9-L1'; 'H-L2 '; 'H-L1 '; 'C9-L2'];
labs = labs([3,2,1,4],:)
legend([pa pb pc pd], labs,'box','off')
...
Fstat = tbl{2,5};
% add to plot anova p value
text(0.75,25,strcat({'p = '},num2str(round(p,3))),'FontSize',16)
% post hoc tukey
% multcompare(stats)% last column is p value, first 2 columns indicate
% which groups are being compared
[p,tbl,stats] = anova1(FR_35,grp'); close(gcf); close(gcf)
Fstat = tbl{2,5};
% add to plot anova p value
text(1.75,14,strcat({'p = '},num2str(round(p,3))),'FontSize',16)

[p,tbl,stats] = anova1(FR_40,grp'); close(gcf); close(gcf)
Fstat = tbl{2,5};
% add to plot anova p value
text(2.75,4,strcat({'p = '},num2str(round(p,3))),'FontSize',16)
set(gca,'XMinorTick','off','YMinorTick','on');
% saveas(f1,strcat('FR_bySD_byCellLine','.png'));
% saveas(f1,strcat('FR_bySD_byCellLine','.epsc'));
...
%% STTC * STTC_synthetic * cell lines as separate lines REORDERED
clear all
files = dir('*FTD*mSpikes_3_CTRL_adjM_0.175.mat');
active_only = 1; %set to one to include on channels with connections
...
A = [scores1{3,1}; scores2{3,1}];
B = [scores1{2,1}; scores2{2,1}];
C = [scores1{1,1}; scores2{1,1}];
D = [scores1{4,1}; scores2{4,1}];
E = [scores1{5,1}; scores2{5,1}];
...
xlim([0.5 2.5])
xticks(1:2)
xticklabels({'Observed';'Synthetic'})
% xlabel('multiplier')
ylabel('global functional connectivity')
a = gca;
a.FontName = 'Arial';
...
x = [0.8:0.1:1.2]';
y = 0.38 * ones(length(x),1);
plot(x,y,'-k','LineWidth',1); text(x(1),0.39,strcat({'p = '},num2str(bwGroupsANOVA_real_p)),'FontSize',16);
x = [1 2]';
y = 0.40 * ones(length(x),1);
plot(x,y,'-k','LineWidth',1); text(x(1),0.41,strcat({'p < '},num2str(mainEffectSource)),'FontSize',16);
...
x = [0.8:0.1:1.2]';
y = 9.3 * ones(length(x),1);
plot(x,y,'-k','LineWidth',1); text(x(1),9.65,strcat({'p = '},num2str(bwGroupsANOVA_real_p)),'FontSize',16);
x = [1 2]';
y = 10 * ones(length(x),1);
plot(x,y,'-k','LineWidth',1); text(x(1),10.35,strcat({'p = '},num2str(mainEffectSource)),'FontSize',16);

labs = ['C9-L1'; 'H-L2 '; 'H-L1 '; 'C9-L2'];
labs = labs([3,2,1,4],:)
l=legend([pa pb pc pd], labs,'box','off');
l.Location = 'north';
% std(A(1,:))) / (sqrt(length(A(1,:)))
set(gca,'XMinorTick','off','YMinorTick','on');
...
%% STTC over dt windows REORDERED
f1 = figure;
hold on
timewins = 1:size(w,2);
% error bars
...
% legend([pa pb pc pd pe], ['CS30   '; 'EpiC   '; 'WTsli32'; 'CS29   '; 'CS30old'])
L1 = plot(10  + timewins,group_means(3,:),...
        '-s','Color',colours{3,1},'MarkerFaceColor',colours{3,1},'MarkerEdgeColor','k',...
        'MarkerSize',10,'LineWidth',2,'Visible','on');
L2 = plot(10  + timewins,group_means(2,:),...
        '-s','Color',colours{2,1},'MarkerFaceColor',colours{2,1},'MarkerEdgeColor','k',...
        'MarkerSize',10,'LineWidth',2,'Visible','on');
L3 = plot(10  + timewins,group_means(1,:),...
        '-s','Color',colours{1,1},'MarkerFaceColor',colours{1,1},'MarkerEdgeColor','k',...
        'MarkerSize',10,'LineWidth',2,'Visible','on');
L4 = plot(10  + timewins,group_means(4,:),...
        '-s','Color',colours{4,1},'MarkerFaceColor',colours{4,1},'MarkerEdgeColor','k',...
...
labs = ['C9-L1'; 'H-L2 '; 'H-L1 '; 'C9-L2'];
labs = labs([3,2,1,4],:)
leg1 = legend([L1 L2 L3 L4],labs,'Location','northwest',...
    'box','off');
% set(leg1, 'SelectionHighlight', 'off');
% https://uk.mathworks.com/matlabcentral/answers/233998-set-legend-to-non-transparent
% std(A(1,:))) / (sqrt(length(A(1,:)))
...
xticks(1:4)
xticklabels({'125';'150';'175';'200'})
xlabel('Synchronicity window, {\it{\Deltat}} (ms)')
ylabel('Mean STTC')
a = gca;
a.FontName = 'Arial';
a.FontSize = 16;
