...
files = dir('*MPT*cSpikes_L0.mat');  % where your .mat files are
...
files = files(~contains({files.name}, 'SMPT'));
...
try
    ...
    disp('burst detection failed')
    output(file).CVIBI = NaN; %3 decimal places
    output(file).BurstRate= NaN;
    output(file).frac_in_burst = NaN;
end
...
clear channels   edges    tarly 
        ...
        method = 'cwt';
        threshold = '0';
        fileName = strcat(method, threshold, '_MPT2020-04', '.mat'); 
