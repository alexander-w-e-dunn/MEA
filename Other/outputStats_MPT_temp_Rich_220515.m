%% Output stats assumes that spike matrices and adjacency matrices have been calculated previously. Use MEA_Analysis to perform these steps first.

clear all; close all

cd 'C:\Users\alexd\OneDrive - University Of Cambridge\Cam\PhD\Project\Data&CodeBackup\AnalysisOutput\By_culture\MPT\SpikesOnwards' % Change to folder with .mat files with spike matrices and adjacency matrices 
files = dir('*2206*Spikes*mat');

%files = files(~contains({files.name}, 'L1')); % Excludes all files from a given MEA culture, in this cirumstance from Left (L) or Right (R) hippocampus, with the experimental repeat indicated (e.g. L1). 
%files = files(~contains({files.name}, 'L2'));
%files = files(~contains({files.name}, 'L3'));
%files = files(~contains({files.name}, 'L4'));
%files = files(~contains({files.name}, 'L5'));
%files = files(~contains({files.name}, 'L6'));
%files = files(~contains({files.name}, 'R1')); 
%files = files(~contains({files.name}, 'R2'));
%files = files(~contains({files.name}, 'R3'));
%files = files(~contains({files.name}, 'R4'));
%files = files(~contains({files.name}, 'R5'));
%files = files(~contains({files.name}, 'R6'));

%files = files(~contains({files.name}, 'DIV07')); % Excludes all files from a given time point
%files = files(~contains({files.name}, 'DIV10'));
%files = files(~contains({files.name}, 'DIV14'));
%files = files(~contains({files.name}, 'DIV17'));
%files = files(~contains({files.name}, 'DIV21'));
%files = files(~contains({files.name}, 'DIV24'));
%files = files(~contains({files.name}, 'DIV28'));
%files = files(~contains({files.name}, 'DIV31'));
%files = files(~contains({files.name}, 'DIV35'));
%files = files(~contains({files.name}, 'DIV38'));
%files = files(~contains({files.name}, 'DIV42'));
%files = files(~contains({files.name}, 'DIV45'));
%files = files(~contains({files.name}, 'DIV49'));
%files = files(~contains({files.name}, 'DIV52'));
%files = files(~contains({files.name}, 'DIV56'));

% files = files(~contains({files.name}, 'TTX','IgnoreCase',true)); % Removes files from a non-developmental recording/experiment.
% files = files(~contains({files.name}, 'Baseline'));
% files = files(~contains({files.name}, 'Drug'));
% files = files(~contains({files.name}, 'IFN'));
% files = files(~contains({files.name}, 'APV'));
% files = files(~contains({files.name}, 'Light')); 
% files = files(~contains({files.name}, 'Stim')); 
% files = files(~contains({files.name}, 'Test'))
% 
% files = files(~contains({files.name}, 'OPAP_220114_2P2_DIV31_mSpikes_ref_1_SDm_5')); % Excludes specific recordings
% files = files(~contains({files.name}, 'OPAP_220321_1L3_DIV38_mSpikes_ref_1_SDm_5'));
% files = files(~contains({files.name}, 'OPAP_220321_2R1_DIV42_mSpikes_ref_1_SDm_5'));
% files = files(~contains({files.name}, 'WT_220131_2Q1_DIV24_22_mSpikes_ref_1_SDm_5'));
% files = files(~contains({files.name}, 'WT_220131_2Q1_DIV24_22_mSpikes_ref_1_SDm_5'));
% 
sampling_fr = 25000;
sync_win_s = 0.01;
...
for file = 1:length(files)
    ...
    tic
    file = files(1)    
    data=load(file.name,'*Spikes','channels','thresholds');
    ...
    try
        mSpikes=data.mSpikes; % Originally mSpikes
        disp('loaded mSpikes')
    catch
        mSpikes=data.mSpikes; % Originally cSpikes
        disp('loaded mSpikes')
    end
    ...
    spikeMat=full(mSpikes); % Originally cSpikes
    ...
    active_chanIndex=find(spikeCounts>=8);
    ...
    disp('spike stats done')
     ...
     %% get channel burst info
    if spikes_only ~= 1
        if elec_burst_stats == 1;
            [burstData] = fcn_bursts_within_bakkum(raster,N,samplingRate);
        end
    end 

    %% get network burst info
    if spikes_only ~= 1
        if net_burst_stats == 1;
            %turn warning off
            ...
            N = 15; minChan = 3; % Changed to 25, could keep at 50 with a larger ISI_N;
            
            spikeMatrix = spikeMat; % Added RT
            minChannel = minChan; % Added RT
        ...
        if length(active_chanIndex)<3 %set all output to 0 etc. if no activity
            ...
            %clear spikeMat
            clear cSpikes mSpikes rSpikes eSpikes
...
%                     buEdges(find(buEdges<0.0001))=0;
    ...
    clearvars -except files file spikes_only burst_stats cor_ctrl ...
        fc_figures g_figures g_measures sampling_fr output sync_win sync_win_s
...
%% save
disp('saving...')
method = 'ExtractedNoise';
threshold = 'Variable';
fileName = strcat(method,'_' ,threshold, 'N15_output_with_cont_and_g_measures_N100_Bursting', '.mat');
