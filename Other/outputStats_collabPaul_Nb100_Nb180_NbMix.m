...
% scriptsDir      = '/nfs/st01/hpc-cbi-sje30/awed2/scripts/Alex-PhD';
...
% cd('/nfs/st01/hpc-cbi-sje30/awed2/Nanobody')

% files = dir('*MPT*cSpikes_L-0.0627_RF1.mat');
files = dir('WT*Spikes*.mat');

files = files(~contains({files.name}, 'adjM','IgnoreCase',true));
...
%         'PRE_3P0','POST_3P0','PRE_10','POST_10','FOLLOWUP'};
    condition_str_idx  = [strfind(files(file).name,'_DIV')+13 : strfind(files(file).name,'Spikes')-3 ];
    conditions = {  '0P0','PRE_0P1','PST_0P1','PRE_0P3','PST_0P3',...
        'PRE_1P0','PST_1P0','PRE_3P0','PST_3P0','PRE_10P','PST_10P','FLP'};
    
    output(file).condition = files(file).name(condition_str_idx);
    
    related_files = files(~~contains({files.name}, files(file).name(1 : strfind(files(file).name,'_DIV'))  ));
    
    if ~isempty(related_files(~~contains({related_files.name}, 'NbStB')))
        output(file).grp = ['NbStB'];
        output(file).grp_num = 1;
        output(file).condition_num = find(contains(conditions,output(file).condition));
    elseif ~isempty(related_files(~~contains({related_files.name}, 'NbMix')))
        output(file).grp = ['NbMix'];
        output(file).grp_num = 4;
        output(file).condition_num = find(contains(conditions,output(file).condition));
    elseif  ~isempty(related_files(~~contains({related_files.name}, 'Nb100')))
        output(file).grp = ['Nb100'];
        output(file).grp_num = 5;
        output(file).condition_num = find(contains(conditions,output(file).condition));
    elseif  ~isempty(related_files(~~contains({related_files.name}, 'Nb183')))
        output(file).grp = ['Nb183'];
        output(file).grp_num = 6;
        output(file).condition_num = find(contains(conditions,output(file).condition));
    end
    clear related_files
...
method = 'mSpikes';
threshold = '5SD';
fileName = strcat(method,'_' ,threshold, '_Nanobody_analysis_2023_Feb_frONLY', '.mat');
