...
% some of below not saving correctly when there is a dot in the file name
% before the file extension. Append the suffix '.epsc' to the filename
% string in addition to specifying the filetype at the end?
% saveas(F1,strcat(filename(1:end-4),'_FireRateXedgenetwork_',scale,'1','.epsc'));
% saveas(F1,strcat(filename(1:end-4),'_FireRateXedgenetwork_',scale,'2','.epsc'),'epsc');
% saveas(F1,strcat(filename(1:end-4),'_FireRateXedgenetwork_',scale,'3','.epsc'),'epsc2');
% saveas(F1,strcat(filename(1:end-4),'_FireRateXedgenetwork_',scale,'4','.svg'),'svg');
