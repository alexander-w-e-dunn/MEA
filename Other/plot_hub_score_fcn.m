...
filename = '200114_FTDOrg_GrpA_5A_Slice1_mSpikes_3_adjM_0.175.mat';

function plot_hubscore_fcn(directory, filename, edge_thresh, node_thresh)
...
load(filename);
if ~exist(strcat(filename(1:end-4),'_hub_score.png'))
...
if strcmp(node_thresh,'average')
    for cutoff = [0.3 0.5 0.7]
        ...
        L_eff_Vec(:,count1)       = efficiency_bin(edges,1); % second input: 1 is local eff. 0 is global
        pcoef_Vec(:,coutn1)       = participation_coef(edges,,0);%input 3: 0 means binary undirected graph
...
saveas(F1,strcat(filename(1:end-4),'_hub_score.png'));
close(F1)
end
end
