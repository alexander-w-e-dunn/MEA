...
elseif strcmp(colourscheme,'black')
    for elecA = 1:length(channels)
        for elecB = 1:length(channels)
            if badjM1(elecA,elecB) >= Emin & elecA ~= elecB;
                p = plot([xc(elecA),xc(elecB)],9-[yc(elecA),yc(elecB)],'LineWidth',...
...
if strcmp(colourscheme,'colour')
    for elecA = 1:length(channels)
        for elecB = 1:length(channels)
            if CIJtree(elecA,elecB) >= Emin & elecA ~= elecB;
                p = plot([xc(elecA),xc(elecB)],9-[yc(elecA),yc(elecB)],'LineWidth',...
                    min_ew + (max_ew-min_ew)*((CIJtree(elecA,elecB)-Emin)/(1-Emin)),'Color',0.5*[1 1 1]);
                % set transparency of lines
                p.Color(4) = 0.2;
            end
        end
...
for i = 1:length(xc)
    plot(xc(i),9-yc(i),'ok','MarkerSize',2*sqrt(1+degree2(i)),...
        'MarkerFaceColor',colours(i,:))
end
% add legend and adjust offset 
nodemin = 1; nodemid = 10; nodemax = 50; 
