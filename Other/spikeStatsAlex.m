...
for i=1:length(files)
    ...
    if files(i).name(14:18)=='DIV07'
        DIV07Spikes(i)=full(sum(sum(tSpikes)));
    elseif files(i).name(14:18)=='DIV14'
        DIV14Spikes(i)=full(sum(sum(tSpikes)));
    elseif files(i).name(14:18)=='DIV21'
        DIV21Spikes(i)=full(sum(sum(tSpikes)));
    elseif files(i).name(14:18)=='DIV28'
        DIV28Spikes(i)=full(sum(sum(tSpikes)));
    else
    end             
end 
    ...
    S_over_dev=round([DIV07Spikes,DIV14Spikes,DIV21Spikes,DIV28Spikes]);
    
%plot
figure
bar(S_over_dev);
xticklabels([07,14,21,28])
...
for i=1:length(files)
    ...
    if files(i).name(14:18)=='DIV14'
        DIV14Spikes_d=full(sum(tSpikes));
...
load('MPT190403_6C_DIV14.mat')
dat=dat(:,1);
[spikeTrain, finalData, threshold] = detectSpikes(dat, 'Tim', 12, 0);

sp_times=find(spikeTrain==1);
figure
for i=1:length(sp_times)
    sp_peak_time=find(finalData(sp_times(i):sp_times(i)+25)==max(finalData(sp_times(i):sp_times(i)+25)));%note changed min to max as with filtered data spike is +ve
...
xticklabels(linspace(0,60,7)/25);
xlabel('time (ms)');
ylabel('uV (filtered & transformed');
%calculate the average and overlay that onto the figure as a red line
%surrounded by white
