...
%
% inputs: 
%     option: 'spikiest', 'diagonal' or a vector of channel xy coordinates e.g. [22 47]

function plotSpikes_fcn(fileName,method,parameter,refPeriod_ms,option,rawDataDir,spikeDir)
...
if strcmp(option,'spikiest')
    ...
    [~,electrode_to_plot]   = maxk(sum(spikeMatrix),3)
...
elseif isvector(option)
    for iii = 1:length(option)
        electrode_to_plot(iii) = find(channels == option(iii));
    end
    ...
    if          ~~exist(strcat(fileName(1:end-4),'_channelXY_',int2str(channels(electrode_to_plot(ei))),...
        ...
        if  ~~exist(strcat(raw_dat_fileName(1:end-4),'_Filtd.mat'))
            ...
            if contains(fileName,'TTX') | contains(fileName,'ttx')
                if contains(fileName,'BEFORE')
                    % don't reset the multiplier if before ttx
                else
                    multiplier = thresholds(electrode_to_plot(ei));
                end
            end
            [spikeTrain, finalData, threshold] = detectSpikes(dat(:,electrode_to_plot(ei)),method, multiplier,L,fs,refPeriod_ms);
        end
        ...
        if sum(spikeTrain) < 5
            ...
            if ~~exist(strcat(fileName(1:end-4),'_channelXY_',int2str(channels(electrode_to_plot(ei))),...
                ...
                plot(x1(sp_times),y2,'v','MarkerSize',5,'MarkerFaceColor','r','MarkerEdgeColor','r');
