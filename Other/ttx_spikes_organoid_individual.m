...
% filesnames = {'200127_FTDOrg_GrpD_5B_Slice7_mSpikes_4.5.mat';... % this will be on y axis
%     '200127_FTDOrg_GrpD_5B_Slice7_mSpikes_5.mat'};                  % this will be on x axis
...
files = dir('191210_slice1_DIV_g04_2018*_2min*');
files = files(~contains({files.name},'ttx','IgnoreCase',true));
files = files(~contains({files.name},'stim','IgnoreCase',true));
files = files(~contains({files.name},'cleaned','IgnoreCase',true));
files = files(~contains({files.name},'190814','IgnoreCase',true));
% files = files(~contains({files.name},'_2min','IgnoreCase',true));
filesnames = {files.name}'; clear files
...
for file = 1:length(filesnames)
    ...
    prefile = fileName(1:strfind(lower(fileName),'spikes')-3) ;
    possiblefiles = dir([prefile,'*.mat']);
    possiblefiles = possiblefiles(~~contains({possiblefiles.name},'ttx','IgnoreCase',true));
    possiblefiles = possiblefiles(~contains({possiblefiles.name},'Spikes'));
    
    try
    postfile = possiblefiles(1).name;    
    
    L=0; refPeriod_ms = num2str(fileName(strfind(fileName,'RF')+2 : end-4));
    
    if ~~isempty(refPeriod_ms)
        refPeriod_ms = 1;
    end
...
%% method comparison plot, x = threshold, y = template
